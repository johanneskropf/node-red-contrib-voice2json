...
Reasons for this could be to see the example sentences that come with a profile or to import the sentences or/and slots from an exeisting profile that you made on another machine and imported here. This also handy to get your profile back into sync should you have made manual changes to the sentences or slots that you would otherwise manage from nodered.
**If you dont load the sentences/slots from the profile and there are already pre existent ones or you made external changes the node will on deploy overwrite the sentences with the nodered version but not the slots!**
